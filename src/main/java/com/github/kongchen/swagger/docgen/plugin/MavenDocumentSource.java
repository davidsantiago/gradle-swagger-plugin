...
public class MavenDocumentSource extends AbstractDocumentSource {
    ...
    public MavenDocumentSource(ApiSource apiSource, Logger logger) {
        super(new LogAdapter(logger), apiSource);
    }

    @Override
    public void loadDocuments(ClassLoader classLoader) throws GenerateException {
        if (apiSource.getSwaggerInternalFilter() != null) {
            try {
                LOG.info("Setting filter configuration: " + apiSource.getSwaggerInternalFilter());
                FilterFactory.setFilter((SwaggerSpecFilter) Class.forName(apiSource.getSwaggerInternalFilter())
                        .newInstance());
            } catch (Exception e) {
        ...
        swagger = reader.read(apiSource.getValidClasses(classLoader, LOG));
        ...
        if (FilterFactory.getFilter() != null) {
            swagger = new SpecFilter().filter(swagger, FilterFactory.getFilter(), new HashMap<String, List<String>>(),
